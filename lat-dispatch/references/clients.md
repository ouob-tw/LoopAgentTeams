...
| qa_executor   | 驗收測試       | codex-tui  | gpt-5.5 | high        | danger-full-access | 驅動真實 app、依 QA 清單寫 E2E 測試驗收、不修改實作碼、使用者可介入 |
...
- `--name <agent_id>` 指定 session 名稱供恢復用。`agent_id` 格式為 `<phase>_<round>_<task_id>`（如 `spec_reviewer_1_2026-05-18-user-api-spec`），round 固定帶（從 1 起算）。
...
```bash
zmx run cc-<name> -d bash -c 'claude --name <agent_id> --model=<model> --effort <effort> --permission-mode <permission> "<prompt>"'
```
...
while sleep ${STALL:-900}; do
  ...
  head -3 .lat/results.yaml 2>/dev/null | grep -q "$AID" && {
    echo "DONE: $AID"; zmx history "$SESSION" | tail -20; break
  }
...
完成判定：`results.yaml` 前幾行包含已派發的 `agent_id`。
...
查詢 session ID（從 session 檔案搜尋 agent_id，UUID 從檔名取）：

```bash
grep -l "<agent_id>" ~/.codex/sessions/$(date -u +%Y/%m/%d)/*.jsonl | head -1 | grep -oP '[0-9a-f]{8}(-[0-9a-f]{4}){3}-[0-9a-f]{12}'
```
