...
- [ ] 整合：Orchestrator 協調整合，由執行 Agent 在整合後版本驗證各 Ticket 的驗收條件與跨 Ticket 限制。
  ...
  - [ ] 整合驗證通過後，關閉沒有後續任務的執行 Agent，重新計算可開始的 Tickets，
        將新解除阻塞的項目直接回到「派發」。所有 Tickets 完成前，流程不停在階段交接點。
...
Grilling 時與使用者共同討論。Spec 確認後，在已確認的需求、範圍與驗收條件內，自動完成拆票、實作、測試、審查、修正與交付。未回覆不視為同意；Orchestrator 僅能在既有授權內裁決。既有授權內的工作不需徵得同意，為這類例行工作徵詢許可與越權裁決同樣是偏離流程。改變已確認需求或超出授權的事項，先保留待確認，其他不受影響的工作可繼續。
