...
讓 LAT 的 writer／reviewer client 在需要查核 library、framework、SDK、API、CLI 或 cloud service 最新文件時，可直接使用使用者已配置的 Context7 MCP。

本功能只增加 Context7 MCP 能力，不限制或改變 client 原有的 sandbox、內建工具、其他 MCP 與權限。

## 適用角色

下列外部 client phase 必須能看見並呼叫 `context7` MCP：

- `spec_reviewer`
- `plan_writer`
- `plan_reviewer`

`spec_writer` 由 Dispatch 自行處理，不在本次外部 client 啟動範圍。`code_executor`、`test_executor` 與 `qa_executor` 也不在本次範圍。
...
writer／reviewer 沿用 client 的有效 MCP 設定。若使用者已在全域或專案設定配置並啟用 `context7`，LAT 不需額外注入啟動參數，也不重複建立同名 server；LAT 只需在角色能力契約與 prompt 中指示有需要時使用 Context7 MCP。
...
| `plan_reviewer` | `read-only` | report-only，不修改計劃 |
...
**預期：** 三個 canonical prompt 都要求在需要時使用既有 Context7 MCP；不要求安裝 CLI。
...
- Claude Code 與 Codex 的三個 writer／reviewer phase 都能使用既有 Context7 MCP。
