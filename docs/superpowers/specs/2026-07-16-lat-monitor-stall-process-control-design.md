...
同意。`spec_reviewer`、`plan_writer` 與使用者明確覆蓋為外部 client 的 `plan_reviewer` 共用：
...
**使用者行為：** 以內建設定啟動 spec reviewer、plan writer，或明確覆蓋為外部 client 的 plan reviewer，並讓 Monitor 重新掛載一次。
