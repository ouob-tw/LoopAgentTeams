...
Claude Code dispatch 以 Monitor 工具執行迴圈時，DRIFT_CHECK 後重設 `START` 繼續；Codex dispatch 以 `exec_command` 執行時，DRIFT_CHECK 後 `break`，由 agent 決定是否 resume（詳見 client.md 的「監控方式」）。
