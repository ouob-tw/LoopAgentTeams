...
`POLL` 是檢查頻率；`STALL` 是多久沒有進展才視為停滯；`DRIFT` 是多久提醒 Dispatch 檢查方向；`yield_time_ms` 是 Dispatch 單次等待 Monitor 輸出的時間。`write_stdin` 空輪詢的工具上限為 300000 毫秒；Codex Dispatch 取 `min(stall_ms, 120000, tool_max_yield_ms)`，並讓外層 `functions.exec` 與內層 `write_stdin` 使用相同的 120000。單次等待結束後接續同一個 exec session，不重啟 Monitor。
