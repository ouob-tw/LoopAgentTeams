---
name: agent-invoke
description: Use when a user asks to delegate work to a Codex or Claude Code agent, continue or stop one exact agent task, select native versus external execution, or manage its exact operation identity.
---
...
3. native：目前 client 支援目標家族的內建 Agent 時，採原生路徑。
4. cross-family exec：僅在使用者給予 explicit consent、原生路徑不可用，且未要求 TUI 時才可使用跨家族 client exec。
5. TUI：僅在使用者明確要求互動式或持久 TUI，且已給予 external consent 時才可使用。
...
- exact resume 或 imported exact reference：讀取 `references/resume.md`；需要狀態、封存、stop 或 clean 判定時，同時讀取 `references/lifecycle.md`。
